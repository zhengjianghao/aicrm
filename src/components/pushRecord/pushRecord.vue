<template>
  <table-list getData='getPushRecord' tableName='推送记录' :sortBy="sortBy" :showExport='true' :headerData='partItem' :showQuickSearchByDays="true" :showDateRangeSearch="true" :showSearchKey="true" dateRangeSearchPlaceholder="请选择营销时间段" @getBaseList="searchRemote" @getFilterList="searchLocal" ref="tableList" :filterListData="pushRecordData">
    <template scope='scope' slot="tableData">
      <el-table :data="scope.remotePageData" max-height="560" stripe border>
        <el-table-column
        v-for="(data, key, index) in partLabel"
        :key='index'
        header-align="center"
        :align="data.type == 'money' ? 'right' : 'center'"
        :min-width="data.width ? data.width : '80'"
        :render-header="utils.appendTip(g_const.table_tooltip[data.name])"
        :label="data.name">
...
export default {
  ...
  watch: {
    subState() {
      // 只有在被激活，才主动获取数据
      if (this.subState === this.g_const.activeCode.pushRecord && this.isActived) {
        this.init()
      }
    }
  },
  methods: {
    ...
    searchRemote([pagination, days, startDate, endDate, search_word, sortBy]) {
      ...
      // 主动获取数据
      this.getPushRecord([pagination, this.taskId, days, startDate, endDate, search_word])
    },
    // 处理本地数据
