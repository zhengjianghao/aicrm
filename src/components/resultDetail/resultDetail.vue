<template>
  <div>
    <table-list getData='getResultDetail' tableName='成果详单' :sortBy="sortBy" :showExport='true' :headerData='partItem' :showQuickSearchByDays="true" :showDateRangeSearch="true" :showSearchKey="false" dateRangeSearchPlaceholder="请选择营销时间段" @getBaseList="searchRemote" @getFilterList="searchLocal" ref="tableList" :filterListData="resultDetailData">
        ...
        <el-table :data="scope.remotePageData" max-height="560" stripe border>
          <el-table-column v-for="(data, key, index) in partLabel" :key='index' header-align="center" :align="data.type == 'money' ? 'right' : 'center'" :min-width="data.width ? data.width : '80'" :render-header="utils.appendTip(g_const.table_tooltip[data.name])" :label="data.name">
            <template scope="scope">
              ...
              <p v-else-if="data.type === 'percent'">{{utils.format_per(scope.row[key])}}</p>
              <p v-else-if="data.type === 'moneyMin'">{{utils.format_money(scope.row[key], 3)}}</p>
...
export default {
  ...
  watch: {
    subState() {
      // 只有在被激活，才主动获取数据
      if (this.subState === this.g_const.activeCode.resultDetail && this.isActived) {
        this.init()
      }
    }
  },
