<template>
  <div class="sub_main_content">
      ...
      <div class="user_tablelist">
          <el-table
              :data="filterResultDetailData.curPageData"
              max-height="560"
              stripe
              border>
              <el-table-column
          ...
          <div class="pull-right">
              <el-pagination
                ...
                :total="filterResultDetailData.totalNum"
...
<script type="text/ecmascript-6" scoped>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['subState','resultDetailData','filterResultDetailData'])
    },
    watch: {
      subState() {
        if (this.subState === globalConstant.activeCode.immRresultDetail) {
            // 只有在被激活，才主动获取数据
    ...
    mounted() {
      this.getResultDetail([this.pagination,3])
    },
    methods: {
      ...mapActions(['getResultDetail','getResultDetailByParam']),
      switchTab(name) {
      ...
      search() {
          this.getResultDetailByParam([this.pagination,this.search_word,this.vm.activeTabName,this.buyDateRang])
      },
      doPageByNum(val) {
