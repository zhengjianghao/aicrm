<template>
  <div class="sub_main_content">
      ...
      <div class="user_tablelist">
          <el-table
              :data="filterUnsubscribeRecordData.curPageData"
              max-height="560"
              stripe
              border>
              <el-table-column
              ...
              min-width="90">
                <template scope="scope">
                  <p>{{utils.format_date(scope.row.unsubscribeTime)}}<br/>{{utils.format_time(scope.row.unsubscribeTime)}}</p>
                </template>
              </el-table-column>
          </el-table>
          <div class="pull-right">
              <el-pagination
                ...
                :total="filterUnsubscribeRecordData.totalNum"
...
<script type="text/ecmascript-6" scoped>
  ...
  export default {
    data() {
      return {
        ...
        tableData3: [{
            ...
            unsubscribeTime: {date: '2017-12-12', time: '12:20'},
            pushState: '已推送',
    ...
    computed: {
      ...mapGetters(['subState','unsubscribeRecordData','filterUnsubscribeRecordData'])
    },
    watch: {
      subState() {
        if (this.subState === globalConstant.activeCode.unsubscribeRecord) {
            // 只有在被激活，才主动获取数据
    ...
    mounted() {
      this.getUnsubscribeRecord([this.pagination,3])
    },
    methods: {
      ...mapActions(['getUnsubscribeRecord','getUnsubscribeRecordByParam']),
      switchTab(name) {
      ...
      search() {
          this.getUnsubscribeRecordByParam([this.pagination,this.search_word,this.vm.activeTabName,this.buyDateRang])
      },
      doPageByNum(val) {
