<template>
  <setting-list-goods ref='settinglistgoods' :settingListData="settingListData" :customUpdate="customUpdate">
    <template slot="toolboxLeft" scope="props">
      <h3 style="color:#5D6D7D;">品牌团宝贝配置</h3>
      <span style="line-height: 30px">此处设置品牌团宝贝折扣规则，并且选择的相似宝贝将被用于精准人群，即AI会根据选择的宝贝来寻找潜需买家</span>
    </template>
    <template slot="toolboxRight" scope="props">
      <div style="padding: 23px 0 0;">
        当前已有
        <span class="num">{{props.data.totalNum ? props.data.totalNum : 0}}</span> 件品牌团宝贝
        <el-button style="background-color: #0bb8f8;background-image: linear-gradient(45deg, #0bb8f8 25%, #2AF598 100%);" type="primary" @click="props.openChoTemplate()">添加品牌团宝贝</el-button>
      </div>
    </template>
            ...
            <div>
              ...
              <el-card class="card_detail" :body-style="{ padding: '10px' }" style="border-bottom-right-radius: 0;border-bottom-left-radius: 0;" v-show="!obj.j_is_edit">
                ...
                <div class="j_body">
                  ...
                  <div class="bottom">
                    <span class="link link_underline primary" @click="props.openChooseSubGoods(obj.num_iid)">添加品牌团宝贝相似宝贝</span>&nbsp;&nbsp;
                    <span class="link link_underline primary" @click="goEdit(props, obj)">品牌团设置</span>
                  </div>
                </div>
              </el-card>
              <!-- 品牌团宝贝相关宝贝 -->
              <div class="card_about nice_scroll" style="width: 99%;max-height: 300px;overflow: auto;background-color: rgba(239, 238, 238, 0.32);box-shadow: 0px 2px 4px 0px rgba(0, 0, 0, .12), 0px 0px 6px 1px rgba(0, 0, 0, .04);border-bottom-right-radius: 4px;border-bottom-left-radius: 4px;">
                <p style='padding: 10px' v-show='obj.j_child_tid.length > 0'>品牌团宝贝相似宝贝</p>
    ...
    checkGoodsPrice(value, id) {
      ...
      if (value == '') {
        if (that.goodspriceID.indexOf(id) < 0) {
          ...
          that.goodspriceContent.push('品牌团价格不得为空')
        } else {
          that.goodspriceContent.splice(that.goodspriceID.indexOf(id), 1)
          that.goodspriceID.splice(that.goodspriceID.indexOf(id), 1)
          that.goodspriceID.push(id)
          that.goodspriceContent.push('品牌团价格不得为空')
          //that.goodspriceContent[that.goodspriceID.indexOf(id)] = '品牌团价格不得为空'
        }
      } else if (!reg.test(value)) {
        if (that.goodspriceID.indexOf(id) < 0) {
          that.goodspriceID.push(id)
          that.goodspriceContent.push('品牌团价格输入错误')
        } else {
          that.goodspriceContent.splice(that.goodspriceID.indexOf(id), 1)
          that.goodspriceID.splice(that.goodspriceID.indexOf(id), 1)
          that.goodspriceID.push(id)
          that.goodspriceContent.push('品牌团价格输入错误')
          //that.goodspriceContent[that.goodspriceID.indexOf(id)] = '品牌团价格输入错误'
        }
      } else {
        if (that.goodspriceID.indexOf(id) >= 0) {
          that.goodspriceContent.splice(that.goodspriceID.indexOf(id), 1)
    ...
    checkGoodsDis(value, id) {
      ...
      if(value === 10 || value === '10') {
        ...
        if (that.goodsdiscountID.indexOf(id) < 0) {
          ...
          that.goodsdiscountContent.push('请输入品牌团折扣')
        } else {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('请输入品牌团折扣')
          //that.goodsdiscountContent[that.goodsdiscountID.indexOf(id)] = '请输入品牌团折扣'
        }
      } else if (!reg.test(value)) {
        if (that.goodsdiscountID.indexOf(id) < 0) {
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('品牌团折扣格式输入错误')
        } else {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('品牌团折扣格式输入错误')
          //that.goodsdiscountContent[that.goodsdiscountID.indexOf(id)] = '品牌团折扣格式输入错误'
        }
      } else {
        if (that.goodsdiscountID.indexOf(id) >= 0) {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
    ...
    checkGoodsDate(value, id) {
      ...
      if (value[1] == 'Invalid date' || value[1] == '') {
        if (that.goodsDateTimeId.indexOf(id) < 0) {
          ...
          that.goodsDateTimeC.push('请选择品牌团结束时间')
        } else {
