<template>
  <div class="sub_main_content">
    <base-marketing-config :service="service" :checkBuyAll="checkBuyAll" :marketingType="g_const.marketTemplateType.HOLIDAYCARE_P4P_SYS" :updateSidebar="getMaintenanceSidebar">
      <template slot="baseConfig" scope="props">
        <!-- 延迟渲染 提升页面跳转速度 -->
        <lazy-render>
          <setting-list ref="settingcheck" :settingListAiData="g_const.holiday_list" :settingListCustomData="(!props.conf.holiday_data_custom || !props.conf.holiday_data_custom[0] || props.conf.holiday_data_custom[0].name === '') ? g_const.holiday_list : props.conf.holiday_data_custom " :holidaySetting="props.conf.holiday_set">
          </setting-list>
        </lazy-render>
        <div class="subTitle">
          条件配置
...
// 头部组件按需加载
// var settingList = Vue.component('settingList', function (resolve) {
//   require(['./settingList'], resolve)
// })
import service from '../holidayCare.service'
export default {
  data() {
    return {
      showMoreBlock: false,
