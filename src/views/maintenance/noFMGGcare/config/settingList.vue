<template>
  <table-list
    ...
    <template slot="toolboxLeft" scope="props">
      <h3 style="color:#5D6D7D;">设置非快消品关怀语</h3>
      <span style="line-height: 30px">对宝贝设置关怀语</span>
    </template>
    ...
    <template slot="tableData" scope="props">
      <el-table :data="props.data" max-height="775" >
        ...
        <el-table-column min-width="220" header-align="center" label="宝贝名称">
          <template scope="scope">
        ...
        <el-table-column prop="num_iid" align="center" min-width="115" label="宝贝ID">
        </el-table-column>
        <el-table-column prop="outer_id" align="center" min-width="120" label="商家编码">
        </el-table-column>
        <el-table-column prop="salesInventory" align="center" min-width="130px" label="销量与库存">
          <template scope="scope">
            <div class="sale">
              <p>销量
        ...
        <el-table-column min-width="180px" align="center" label="关怀语">
          <template scope="scope">
            <el-tooltip
              class="item"
              effect="dark"
              :manual="true"
              :value='buyDayID.indexOf(scope.row.num_iid) >=0 '
              :content="buyDayContent.length >0 ? buyDayID.indexOf(scope.row.num_iid) >=0 ? buyDayContent[buyDayID.indexOf(scope.row.num_iid)] : '' : ''"
              placement="top">
                <el-input v-model="scope.row.care_message" @blur="checkBuyCycelDays(scope.row.care_message, scope.row.num_iid)">
                </el-input>
            </el-tooltip>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    checkBuyCycelDays(value, id) {
        ...
        if(value=='') {
          if(that.buyDayID.indexOf(id)<0){
            ...
            that.buyDayContent.push('关怀语不能为空')
          }else{
          ...
          if(that.buyDayID.indexOf(id)<0){
            that.buyDayID.push(id)
            that.buyDayContent.push('问候语不能超过100个字')
          }else {
            that.buyDayContent.splice(that.buyDayID.indexOf(id),1)
            that.buyDayID.splice(that.buyDayID.indexOf(id),1)
            that.buyDayID.push(id)
    ...
    checkBuyAll() {
      for(var i in this.buyingCycelGoodsCur) {
        this.checkBuyCycelDays(this.buyingCycelGoodsCur[i].care_message, this.buyingCycelGoodsCur[i].num_iid)
      }
    ...
    getBuyingRycelGoodsByInit(initGoods) {
      ...
      return _initGoods.map(o => {
        ...
        // 如果宝贝不为空获取购买周期数
        if (matchObj) {
          matchObj.care_message = o.care_message
        }
        return matchObj
      })
    },
