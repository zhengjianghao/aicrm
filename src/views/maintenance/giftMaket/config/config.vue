<template>
  <div class="sub_main_content">
    ...
    :service="service"
    :marketingType="g_const.marketTemplateType.GIFTMARKET_P4P_SYS"
    ...
    :updateSidebar="getMaintenanceSidebar">
      ...
      scope="props">
        <!-- 延迟渲染 提升页面跳转速度 -->
        <lazy-render>
          <setting-list ref="settingcheck"  :settingListData="props.conf.discount_set">
          </setting-list>
        </lazy-render>
        <div class="subTitle">
          条件配置
        ...
        <lazy-render loadingTime="200">
          <div class="form-control form_control" style="padding-bottom: 0px">
              ...
              <config-sendtime-N
              :sendTimeLabel='props.conf.send_time_label'
              :sendTime='props.conf.send_time'
              :excludeTimeLabel='props.conf.exclude_time_label'
              :elcludeTime="props.conf.exclude_time"
              :sendTimeDay='props.conf.send_time_day'
              :sendWord="'最近一次购物签收后'"
              :aheadTimeLabel="props.conf.ahead_time_label">
              </config-sendtime-N>
              <el-form-item>
              ...
              <div class="subTitle title_align more" @click="showMoreBlock = !showMoreBlock">
                ...
                <span class="history_remarks" v-show="showMoreBlock">注释：{{g_const.orderRemarks[2].label}}</span>
              </div>
...
<script type="text/ecmascript-6" scoped>
  ...
  import service from '../giftMarket.service'
  export default {
    data() {
      return {
        showMoreBlock: false,
    ...
    methods: {
      ...
      checkBuyAll() {
        return this.$refs.settingcheck.checkAll()
      }
    },
    components: {
      configInterval,
