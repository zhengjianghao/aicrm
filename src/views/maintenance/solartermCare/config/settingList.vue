<template>
  <table-list
    ...
    <template slot="toolboxLeft" scope="props">
      <h3 style="color:#5D6D7D;">节气设置</h3>
      <el-radio v-model="holiday_setting" label="">
        <AI></AI> 智慧管理</el-radio>
      <el-radio v-model="holiday_setting" label="custom">自定义节气</el-radio>
    </template>
    <template slot="toolboxRight" scope="props">
      <div style="padding: 25px 10px 0px 0px;">
        已设置
        <span class="num">{{props.data.totalNum ? props.data.totalNum : 0}}</span> 个节气问候
        <el-button style="background-color: #0bb8f8;background-image: linear-gradient(45deg, #0bb8f8 25%, #2AF598 100%);" v-show="canEdit" type="primary" @click="add">添加节气</el-button>
      </div>
    </template>
    <template slot="tableData" scope="props">
      <!-- AI管理 -->
      <el-table v-if="holiday_setting === ''" :data="props.data" max-height="775" border>
        <el-table-column min-width="60px" align="left" prop="festivalName" label="节气名称">
      ...
      <!-- 自定义 -->
      <el-table v-else-if="holiday_setting === 'custom'" :data="props.data" max-height="775" border>
        <el-table-column min-width="60px" align="left" prop="festivalName" label="节气名称">
          <template scope="scope">
        ...
        <el-table-column min-width="110px" prop="festivalDate" align="left" label="节气日期">
          <template scope="scope">
            ...
            <el-date-picker class="input-custom" style="width: 55%; display: inline-block" v-model="scope.row.date" @change="updateValue" :editable="false" type="date" placeholder="选择日期">
            </el-date-picker>
          </template>
        </el-table-column>
        <el-table-column prop="applyArea" align="left" min-width="200" label="适用地区">
