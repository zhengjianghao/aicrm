<template>
  <el-row :gutter="20">
      ...
      <el-col :span="5">
          <span @click="openPushItem('inBuyGoods', items_range.xx)">
              <el-radio v-model="items_range_label" label="xx">指定部分宝贝
                  <span class="info">(已选中{{ items_range.xx ? items_range.xx.length : 0 }}个宝贝)</span>
              </el-radio>
          </span>
      </el-col>
      <el-col :span="14">
          <span @click="openPushItem('outBuyGoods', items_range.xy)">
              <el-radio v-model="items_range_label" label="xy">排除部分宝贝
                  <span class="info">(已选中{{ items_range.xy ? items_range.xy.length : 0 }}个宝贝)</span>
              </el-radio>
          </span>
      </el-col>
  </el-row>
    ...
    watch: {
      ...
      selectGoodsData() {
        ...
        if(selGoodsData.eleName === 'inBuyGoods') {
            this.items_range.xx = selGoodsData.selectGoods
        } else if(selGoodsData.eleName === 'outBuyGoods') {
            this.items_range.xy = selGoodsData.selectGoods
        }
      }
    },
    computed: {
