<template>
  <div class="sub_main_content">
    ...
    :service="service"
    :marketingType="g_const.marketTemplateType.CLUBCARDREMIND_P4P_SYS"
                ...
                <el-form-item>
                  <el-row :gutter="20">
                      ...
                      <el-col :span="5">
                        ...
                        <el-tooltip popper-class='tool_notes' content='注释：魔方墙AI引擎基于最近3个月买家购物行为分析获得，触发时间为会员下次购物时间间隔均值的50%，每月更新动态跟随店铺发展；主要价值用于保持与买家的维系' placement="top-start">
                          <i class="iconfont icon-help1 toolGanTanHao"></i>
                        </el-tooltip>
                      </el-col>
                      <el-col :span="16"><el-radio v-model="props.conf.trigger_node_label" label='custom'>买家确认收货</el-radio>
                        <el-select
                        v-model="props.conf.trigger_node"
                        class="el-input-sm-sm"
                        :disabled="props.conf.trigger_node_label != 'custom'">
                          <el-option
                          v-for="item in g_const.urgeTime3"
                          :label="item.name"
                          :value="item.value"
                          :key="item.value"
                          >
...
<script type="text/ecmascript-6" scoped>
  ...
  import service from '../clubCardRemind.service'
  export default {
    data() {
      return {
        showMoreBlock: false,
