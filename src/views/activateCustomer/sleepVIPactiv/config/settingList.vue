<template>
  <setting-list-goods ref='settinglistgoods' :isCheckEdit='isCheckEdit' :settingListData="settingListData" :customUpdate="customUpdate">
    <template slot="toolboxLeft" scope="props">
      ...
      <span>选择主宝贝后可选择相似宝贝进行关联，魔方墙AI思维引擎会根据相似宝贝匹配有潜需的买家，扩大精准营销范围，并且可以对高中低不同客单价会员进行精准营销</span>
      <el-row :gutter="20">
        <el-col :span="24">
    ...
    <template slot="toolboxRight" scope="props">
      <div style="padding: 40px 0 0;">
        ...
        <el-button style="background-color: #0bb8f8;background-image: linear-gradient(45deg, #0bb8f8 25%, #2AF598 100%);" type="primary" @click="props.openChoTemplate()">添加宝贝</el-button>
      </div>
    </template>
            ...
            <div>
              ...
              <el-card class="card_detail" :body-style="{ padding: '10px' }" style="border-bottom-right-radius: 0;border-bottom-left-radius: 0;" v-show="!obj.j_is_edit">
                ...
                <div class="j_body">
                  ...
                  <div class="bottom">
                    <span class="link link_underline primary" @click="props.openChooseSubGoods(obj.num_iid)">添加活动宝贝相似宝贝</span>&nbsp;&nbsp;
    ...
    checkGoodsDis(value, id) {
      ...
      if (value === 10 || value === '10') {
        ...
        if (that.goodsdiscountID.indexOf(id) < 0) {
          ...
          that.goodsdiscountContent.push('请输入活动宝贝折扣')
        } else {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('请输入活动宝贝折扣')
          //that.goodsdiscountContent[that.goodsdiscountID.indexOf(id)] = '请输入活动宝贝折扣'
        }
      } else if (!reg.test(value)) {
        if (that.goodsdiscountID.indexOf(id) < 0) {
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('活动宝贝折扣格式输入错误')
        } else {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.splice(that.goodsdiscountID.indexOf(id), 1)
          that.goodsdiscountID.push(id)
          that.goodsdiscountContent.push('活动宝贝折扣格式输入错误')
          //that.goodsdiscountContent[that.goodsdiscountID.indexOf(id)] = '活动宝贝折扣格式输入错误'
        }
      } else {
        if (that.goodsdiscountID.indexOf(id) >= 0) {
          that.goodsdiscountContent.splice(that.goodsdiscountID.indexOf(id), 1)
